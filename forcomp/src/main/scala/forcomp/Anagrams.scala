...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    val lowerCase = w.toLowerCase.toList
    val grouped = lowerCase.toList.groupBy(c => lowerCase.count(c == _)).toList
    val brut: List[(Char, Int)] = grouped.flatMap(pair => {
      pair._2.distinct.map((_, pair._1))
    })
    brut.sortBy(_._1)
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    ...
    subAnagrams(Anagrams.sentenceOccurrences(sentence))
  }
}
